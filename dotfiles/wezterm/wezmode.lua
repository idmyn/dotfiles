-- from https://github.com/twilsoft/wezmode
...
____modules = {
        ...
        do
            ...
            __TS__Promise.prototype["then"] = function(self, onFulfilled, onRejected)
                ...
                if onFulfilled then
                    ...
                    ____self_fulfilledCallbacks_2[#____self_fulfilledCallbacks_2 + 1] = function(____, v)
                        return resolve(
                            nil, v)
                    end
                end
                if onRejected then
                    ...
                    ____self_rejectedCallbacks_4[#____self_rejectedCallbacks_4 + 1] = function(____, err)
                        return reject(
                            nil, err)
                    end
                end
    ...
    ["helpers"] = function(...)
        ...
        ____exports.mergeObjects = function(o1, o2)
            __TS__ArrayForEach(
                __TS__ObjectKeys(o2),
                function(____, k)
                    local v1 = o1[k]
                    ...
                    if isObject(v1) and isObject(v2) then
                        o1 = __TS__ObjectAssign(
                            o1,
                            { [k] = ____exports.mergeObjects(v1, v2) }
                        )
                    else
                        o1 = __TS__ObjectAssign(o1, { [k] = o2[k] })
                    end
                end
            )
            return o1
    ...
    ["wm"] = function(...)
        ...
        local mergeObjects = ____helpers.mergeObjects
        local state = { keys = {}, modeTexts = {}, keyTables = {} }
        local defaultOpts = {
            modifier = "CTRL",
            hintSeparator = "/",
            theme = {
        ...
        local function createPrefixText(prefix, textColor, intensity)
            return wezterm.format({ { Attribute = { Intensity = intensity } }, { Foreground = { Color = textColor } }, { Text = prefix } })
        end
        local function createHintText(key, desc, hintColor, textColor, intensity)
        ...
        local function createModeText(name, textColor, modeColor, intensity)
            return wezterm.format({
                { Attribute = { Intensity = intensity } },
        ...
        local function setup(modes, opts)
            local options = mergeObjects(defaultOpts, opts or ({}))
            local expandedModifier = (function()
                repeat
                    local ____switch7 = options.modifier
                    local ____cond7 = ____switch7 == "MEH"
                    if ____cond7 then
                        return "ALT|CTRL|SHIFT"
                    end
                    ____cond7 = ____cond7 or (____switch7 == "HYP" or ____switch7 == "HYPER")
                    if ____cond7 then
            ...
            local modeHints = table.concat(
                ...
                (" " .. options.hintSeparator) .. " " or ","
            )
            state.modeTexts.normal = (((createPrefixText(options.modifier, options.theme.normalModeColor, options.theme.intensity) .. " + ") .. modeHints) .. " ") ..
                createModeText("normal", options.theme.modeTextColor, options.theme.normalModeColor,
                    options.theme.intensity)
            __TS__ArrayForEach(
                modes,
                function(____, m)
                    local ____state_keys_0 = state.keys
                    ____state_keys_0[#____state_keys_0 + 1] = {
                        key = m.key,
                        mods = expandedModifier,
                        action = wezterm.action.ActivateKeyTable({
                            name = m.name,
                            one_shot = not not m.one_shot,
                            until_unknown = not not
                                m.until_unknown,
                            prevent_fallback = not not m.prevent_fallback
                        })
                    }
                    state.keyTables[m.name] = m.keyTable
                    local actionHints = table.concat(
                        __TS__ArrayMap(
                            m.keyTable,
                            function(____, k)
                                return createHintText(
                    ...
                    state.modeTexts[m.name] = (actionHints .. " ") ..
                        createModeText(m.name, options.theme.modeTextColor, m.modeColor, options.theme.intensity)
                end
            )
        end
        local function getModeText(modeName)
            return state.modeTexts[modeName]
        end
        local function getKeys()
            return state.keys
        end
        local function getKeyTables()
            return state.keyTables
        end
        local function handleRightStatusUpdate()
            wezterm.on(
                "update-right-status",
                function(window)
                    window:set_right_status(getModeText(window:active_key_table() or "normal"))
                end
            )
        end
        local mergeTables = mergeObjects
        local function extendTable(t1, t2)
            return __TS__ArrayConcat(t1, t2)
        end
        ____exports.setup = setup
        ____exports.getModeText = getModeText
        ____exports.getKeys = getKeys
        ____exports.getKeyTables = getKeyTables
        ____exports.handleRightStatusUpdate = handleRightStatusUpdate
        ____exports.mergeTables = mergeTables
        ____exports.extendTable = extendTable
        return ____exports
    end,
}
return require("wm", ...)
